...
type Props = {
  data: any;
  activeVideo?: number; // Spelling fixed
  setActiveVideo?: any; // Spelling fixed
  isDemo?: boolean;
}
...
const CourseContentList: FC<Props> = (props: Props) => {
  ...
  //Find unique video section by its name
  const videoSection: string[] = [
    ...new Set<string>(props.data?.map((item: any) => item.videoSection)) // Fixed: vadioSection -> videoSection
  ];

  let totalCount: number = 0; // total count of video previous section
  
  // toggle section
  const toggleSection = (section: string) => {
    const newVisibleSection = new Set(visibleSection); // copy state from the previous state
    if (newVisibleSection.has(section)) {
      newVisibleSection.delete(section); // if section is  open then close it
    }
    else {
      newVisibleSection.add(section); // if section is close then open it 
    }
    setVisibleSection(newVisibleSection); // change the state 
  }
    ...
    >
      {videoSection.map((section: string, sectionIndex: number) => {
        ...
        const sectionVideos: any[] = props.data.filter(
          (item: any) => item.videoSection === section // Fixed spelling
        );
